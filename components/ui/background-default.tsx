import React from 'react'

type Props = {}

const BackgroundDefault = (props: Props) => {
    return (
        <svg className='rounded-t-md' xmlns="http://www.w3.org/2000/svg" viewBox="0 0 552 138" id="person-default" data-supported-dps="2048x512">
            <path fill="none" d="M0 0h552v138H0z" />
            <path fill="#d9e5e7" d="M0 0h552v138H0z" />
            <path fill="#bfd3d6" d="M380 0h172v138H380z" />
            <path d="M333.22 0H0v138h333.22a207.93 207.93 0 000-138z" fill="#a0b4b7" />
        </svg>
    )
}

export default BackgroundDefault